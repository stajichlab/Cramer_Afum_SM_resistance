#!/usr/bin/bash -l
#SBATCH --mem=64G -p stajichlab --nodes 1 --ntasks 8 --out logs/snpEff.log
module load snpEff/4.3t
module load bcftools
module load yq

# THIS IS AN EXAMPLE OF HOW TO MAKE SNPEFF - it is for A.fumigatus
SNPEFFGENOME=AfumigatusAf293_FungiDB_61
...
SNPEFFOUT=$SNPEFFOUT/$GENOMENAME
mkdir -p $SNPEFFOUT
...
makeMatrix() {
  ...
  mkdir -p $POPNAME
  COMBVCF="$TOPFOLDER/$FINALVCF/$GENOMENAME/$PREFIX.$POPNAME.SNP.combined_selected.vcf.gz $TOPFOLDER/$FINALVCF/$GENOMENAME/$PREFIX.$POPNAME.INDEL.combined_selected.vcf.gz"
  ...
  if [[ ! -s $INVCF || $TOPFOLDER/$FINALVCF/$GENOMENAME/$PREFIX.$POPNAME.SNP.combined_selected.vcf.gz -nt $INVCF ]]; then
